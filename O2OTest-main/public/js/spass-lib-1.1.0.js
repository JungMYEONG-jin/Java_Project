...
//	    {'urls': this.turnUrl+'?transport=tcp', username:'spass', credential:'spass'} 
		{'urls': [ this.turnUrl+'?transport=udp'], username:'spass', credential:'spass'}
	    ,{'urls': [ this.turnUrl+'?transport=tcp'], username:'spass', credential:'spass'}
	  ]
	};
	...
	log.log("swhan debug window.cordova0----------------------------------------["+window.cordova+"]");
	log.log("swhan console window.cordova0----------------------------------------["+window.cordova+"]");
	// 초기화 호출
...
SPass.prototype.startRecording = function() {
	...
	if (this.canvasVideo == null) {
		...
		this.canvasStream = this.canvasVideo.captureStream(15);
	...
	var drawFunction = function () {
		...
		// 배열에 비디오 엘리먼트목록을 만단다.
		var videoList = [$("#video_remote")[0]];

		// 행렬의 최대 크기 구하기
		var userCnt = videoList.length;
		var colSize = 1;
		var rowSize = 1;
		while (true) {
		...
		for (var idx = 0; idx < videoList.length; idx++) {
			var videoObj = videoList[idx];
...
SPass.prototype.uploadMovieFile = function(params, callback) {
	...
	formdata.append("etc3", (params.etc3 ? params.etc3 : ""));
	formdata.append("etc4", (params.etc4 ? params.etc4 : ""));
	formdata.append("idcard_msg", (params.idcard_msg ? params.idcard_msg : ""));
	...
	$.ajax({
		...
		dataType	: 'json',
		success		: function(json){
			log.log('/noface/NF1070.do complete! result.');
			log.log(json);
			if(callback) {
				callback.call(this, json);
			}
