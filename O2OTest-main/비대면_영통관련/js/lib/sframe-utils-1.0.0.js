...
function SFConfig() {
	this.log_level = "debug";  // error < (debug==log) < info < none
	this.devUrl = "devnoface2.shinhan.com";
	this.mobileUrl = "devnoface.shinhan.com";
};
var config = new SFConfig();

//if (getUrlParam("isLog") == "Y" || window.location.href.indexOf("mbstest") != -1) {
...
SFConfig.prototype.isMobile = function() {
	if (window.location.href.indexOf(this.mobileUrl) != -1) {
		return true;
	} else {
		return false;
	}
